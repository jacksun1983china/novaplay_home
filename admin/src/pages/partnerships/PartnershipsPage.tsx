...
import { partnershipApi, uploadApi } from '@/lib/api'
import { toast } from 'sonner'
import { Plus, Pencil, Trash2, Loader2, Upload, Save, X } from 'lucide-react'
...
export default function PartnershipsPage() {
  ...
  const openCreate = () => setEditing({ title: '', subtitle: '', description: '', icon_url: '', sort_order: 0, status: 1 })
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true)
    try {
      const res = await uploadApi.image(file, 'partnerships')
      if (res.data.code === 200) {
        setEditing((prev: any) => ({ ...prev, icon_url: res.data.data.url }))
        toast.success('上传成功')
      }
  ...
  const handleSave = async () => {
    ...
    try {
      let res
      if (editing.id) {
        res = await partnershipApi.update(editing.id, editing)
      } else {
        res = await partnershipApi.create(editing)
      }
      if (res.data.code === 200) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await partnershipApi.delete(id)
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {items.length === 0 && (
            <div className="col-span-3 py-12 text-center text-muted-foreground">暂无数据，点击右上角添加</div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-2xl border border-border w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold">{editing.id ? '编辑合作模式' : '添加合作模式'}</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={editing.subtitle || ''} onChange={e => setEditing((p: any) => ({ ...p, subtitle: e.target.value }))}
                  className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring bg-background" />
              </div>
              <div>
